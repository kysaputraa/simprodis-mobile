...
class ReservoirScreen extends StatelessWidget {
  ...
  List<TextEditingController> tinggiControllers = [];

  @override
  Widget build(BuildContext context) {
    final instalasiCubitState = BlocProvider.of<InstalasiCubit>(context).state;
    final reservoirCubit = ReservoirCubit();
    reservoirCubit.fetchData(idKelompokReservoir: idKelompok);
    String? selectedJam;
    String? selectedTanggal;
    String? selectedInstalasi;
    List<String> idReservoir = [];
            ...
            return SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  BlocConsumer<ReservoirCubit, ReservoirState>(
                    bloc: reservoirCubit,
                    listener: (context, state) {
                      if (state is ReservoirSuccessInsert) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Data berhasil disimpan!')),
                        );
                        Navigator.of(context).pop();
                      } else if (state is ReservoirError) {
                        ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    builder: (context, state) {
                      if (state is ReservoirSuccess) {
                        for (int i = 0; i < state.data.length; i++) {
                          tinggiControllers.add(TextEditingController());
                        }
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            idReservoir.add(pressure.idReservoir!);
                            return Padding(
                                          ...
                                          children: [
                                            ...
                                            Expanded(
                                              ...
                                              child: Container(
                                                ...
                                                child: Text(
                                                  "${state.data[index].namaReservoir}",
                                                  style: TextStyle(
                                                    color: Colors.white,
                                                  ),
                                                ),
                      ...
                      } else if (state is ReservoirLoading) {
                  ...
                  ElevatedButton(
                    onPressed: () {
                      List<String> tinggiList =
                          tinggiControllers
                              .map((controller) => controller.text)
                              .toList();

                      reservoirCubit.simpan(
                        idKelompokReservoir: idKelompok,
                        idReservoir: idReservoir,
                        tinggiList: tinggiList,
                        jam: selectedJam,
