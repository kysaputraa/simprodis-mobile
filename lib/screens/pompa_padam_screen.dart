...
import 'package:simprodis_flutter/bloc/kelompok_pompa_cubit/kelompok_pompa_cubit.dart';
import 'package:simprodis_flutter/bloc/listrik_padam_cubit/listrik_padam_cubit.dart';
import 'package:simprodis_flutter/bloc/poma_padam_cubit/pompa_padam_cubit.dart';
...
class PompaPadamScreen extends StatefulWidget {
  PompaPadamScreen({super.key});

  @override
  State<PompaPadamScreen> createState() => _PompaPadamScreenState();
}

class _PompaPadamScreenState extends State<PompaPadamScreen> {
  DateTime? _selectedDate;
  String? baseUrl = dotenv.env['BASE_URL'];

  late final InstalasiCubit instalasCubit;
  late final PompaCubit pompaCubit = PompaCubit();
  late final KelompokPompaCubit kelompokPompaCubit = KelompokPompaCubit();
  late final PompaPadamCubit pompaPadamCubit = PompaPadamCubit();

  String? selectedKelompokPompa;
  String? selectedPompa;
      ...
      appBar: AppBar(
        ...
        flexibleSpace: Container(
          decoration: BoxDecoration(
            color: const Color.fromARGB(255, 215, 198, 46),
          ),
          child: Center(
            child: Column(
      ...
      body: ListView(
        ...
        children: [
                              ...
                              return DropdownButtonHideUnderline(
                                child: DropdownButton<String>(
                                  ...
                                  onChanged: (String? newValue) {
                                    ...
                                    kelompokPompaCubit.fetchData(
                                      id_instalasi: newValue,
                                    );
                                    selectedKelompokPompa = null;
                                    selectedPompa = null;
                                  },
                                  isExpanded: true,
                                  items:
                  ...
                  Expanded(
                    ...
                    child: Padding(
                      ...
                      child: Container(
                        ...
                        child:
                            BlocBuilder<KelompokPompaCubit, KelompokPompaState>(
                              bloc: kelompokPompaCubit,
                              builder: (context, state) {
                                if (state is KelompokPompaLoading) {
                                  return Center(
                                    child: CircularProgressIndicator(),
                                  );
                                }
                                if (state is KelompokPompaSuccess) {
                                  List<DataKelompokPompa>? data = state.data;
                                  if (data == null) {
                                    return Center(
                                      child: Text(
                                        'Tidak ada Kelompok Pompa tersedia',
                                        style: TextStyle(color: Colors.white),
                                      ),
                                    );
                                  }
                                  return DropdownButtonHideUnderline(
                                    child: DropdownButton<String>(
                                      value: selectedKelompokPompa,
                                      hint: Text(
                                        'Pilih Kelompok Pompa',
                                        style: TextStyle(color: Colors.white),
                                      ),
                                      onChanged: (String? newValue) {
                                        pompaCubit.fetchData(
                                          idKelompokPompa: newValue,
                                        );
                                        setState(() {
                                          selectedKelompokPompa = newValue;
                                          selectedPompa = null;
                                        });
                                      },
                                      isExpanded: true,
                                      items:
                                          data.map<DropdownMenuItem<String>>((
                                            DataKelompokPompa value,
                                          ) {
                                            return DropdownMenuItem<String>(
                                              value: value.idKlpPompa,
                                              child: Text(
                                                value.namaKlpPompa.toString(),
                                              ),
                                            );
                                          }).toList(),
                                      icon: Icon(
                                ...
                                } else if (state is KelompokPompaError) {
                                  return Text('Error: ${state.message}');
                                } else {
                                  return Text('Silahkan Pilih Instalasi');
                                }
                              },
                            ),
                      ),
                  ...
                  Expanded(
                    ...
                    child: Padding(
                      ...
                      child: Container(
                        ...
                        child: BlocBuilder<PompaCubit, PompaState>(
                          bloc: pompaCubit,
                          builder: (context, state) {
                            if (state is PompaLoading) {
                              return Center(child: CircularProgressIndicator());
                            }
                            if (state is PompaSuccess) {
                              List<DataPompa> data = state.data;
                              if (data.isEmpty) {
                                return Center(
                                  child: Text(
                                    'Tidak ada Pompa tersedia',
                                    style: TextStyle(color: Colors.white),
                                  ),
                                );
                              }
                              return DropdownButtonHideUnderline(
                                child: DropdownButton<String>(
                                  ...
                                  onChanged: (String? newValue) {
                                    setState(() {
                                      selectedPompa = newValue;
                                    });
                                    pompaPadamCubit.fetchData(
                                      idPompa: newValue,
                                    );
                                  },
                                  isExpanded: true,
                                  items:
                                      data.map<DropdownMenuItem<String>>((
                                        ...
                                        return DropdownMenuItem<String>(
                                          value: value.idPompa.toString(),
                                          child: Text(
                                            value.namaPompa.toString(),
                                          ),
                                        );
                                      }).toList(),
                                  icon: Icon(
          ...
          BlocBuilder(
            ...
            builder: (context, state) {
              if (state is InstalasiSuccess) {
                return BlocConsumer<PompaPadamCubit, PompaPadamState>(
                  bloc: pompaPadamCubit,
                  listener: (context, statePompaPadam) {
                    if (statePompaPadam is PompaPadamSuccessInsert) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(statePompaPadam.message),
                          backgroundColor: Colors.green,
                        ),
                      );
                    } else if (statePompaPadam is PompaPadamError) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(statePompaPadam.message),
                  ...
                  builder: (context, statePompaPadam) {
                    if (statePompaPadam is PompaPadamLoading) {
                      return Center(child: CircularProgressIndicator());
                    } else if (statePompaPadam is PompaPadamError) {
                      return Center(
                        child: Text('Gagal : ${statePompaPadam.message}'),
                      );
                    } else if (statePompaPadam is PompaPadamSuccess) {
                      return Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                                              ...
                                                              child: BlocBuilder(
                                                                ...
                                                                builder: (
                                                                  ...
                                                                  if (statePompaPadam
                                                                      ...
                                                                      null) {
                                                                    return Text(
                                                                      "${statePompaPadam.data?.waktuPadam}",
                                                                    );
                                                                  } else if (statePompaPadam
                                                                          .selectedWaktuPadam !=
                                                                      null) {
                                                                    return Text(
                                                                      "${statePompaPadam.selectedWaktuPadam}",
                                                                    );
                                                                  } else {
                                                                    return Text(
                                                                      "Pilih Tanggal",
                                ...
                                if (statePompaPadam.data?.waktuPadam != null)
                                  Expanded(
                                    child: Container(
                                      width: 50,
                                      height: 50,
                                        ...
                                        child: InkWell(
                                          onTap: () {
                                            if (statePompaPadam
                                                    .selectedWaktuPadam !=
                                                null) {
                                              pompaPadamCubit.simpan(
                                                id_pompa: selectedPompa,
                                                waktuPadam:
                                                    statePompaPadam
                                                        .selectedWaktuPadam,
                                              );
                                                              ...
                                                              child: BlocBuilder(
                                                                ...
                                                                builder: (
                                                                  ...
                                                                  if (statePompaPadam
                                                                      ...
                                                                      null) {
                                                                    return Text(
                                                                      "${statePompaPadam.selectedWaktuHidup}",
                                                                    );
                                                                  } else {
                                                                    return Text(
                                                                      "Pilih Tanggal",
                                      ...
                                      child: InkWell(
                                        onTap: () {
                                          if (statePompaPadam
                                                      .selectedWaktuHidup !=
                                                  null &&
                                              statePompaPadam.data?.id !=
                                                  null) {
                                            pompaPadamCubit.updateWaktuHidup(
                                              idPompa: selectedPompa,
                                              id_padam:
                                                  statePompaPadam.data?.id,
                                              waktu_hidup_kembali:
                                                  statePompaPadam
